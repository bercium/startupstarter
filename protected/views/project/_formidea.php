 <?php if(Yii::app()->user->hasFlash('personalMessage')){ ?>
   ...
   <?php echo CHtml::beginForm('','post',array('class'=>"custom formidea")); ?>
    ...
    <?php echo CHtml::activedropDownList($translation, 'language_id', GxHtml::listDataEx(Language::model()->findAllAttributes(array("id","native_name"), true, array('order'=>'FIELD(id, '.$language->id.', 40) DESC, native_name ASC')),"id","native_name"), array('style'=>'display: none;')); ?>
    ...
    <?php echo CHtml::activedropDownList($idea, 'status_id', GxHtml::listDataEx(IdeaStatus::model()->findAllAttributes(null, true)), array('empty' => '&nbsp;', 'style' => 'display: none;')); ?>
