...
class m140312_231320_calendar_events extends CDbMigration
{
  ...
  ADD CONSTRAINT `location_ibfk_1` FOREIGN KEY (`country_id`) REFERENCES `country` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;");
        ...
        $this->execute("ALTER TABLE  `location` ADD  `count` INT( 11 ) UNSIGNED NOT NULL ;");

    $this->execute("ALTER TABLE  `location` CHANGE  `country_id`  `country_id` SMALLINT( 3 ) UNSIGNED NULL DEFAULT NULL ;");
    $this->execute("ALTER TABLE  `location` CHANGE  `city_id`  `city_id` INT( 10 ) UNSIGNED NULL DEFAULT NULL ;");
