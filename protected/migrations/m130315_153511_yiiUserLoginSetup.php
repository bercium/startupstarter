...
class m130315_153511_yiiUserLoginSetup extends CDbMigration
{
  ...
  $this->execute("CREATE TABLE `profiles` (
  ...
  $this->execute("ALTER TABLE `profiles` ADD CONSTRAINT `user_profile_id` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE; ");
  ...
  $this->execute("INSERT INTO `profiles` (`user_id`, `lastname`, `firstname`) VALUES
  ...
  $this->execute("INSERT INTO `profiles_fields` (`id`, `varname`, `title`, `field_type`, `field_size`, `field_size_min`, `required`, `match`, `range`, `error_message`, `other_validator`, `default`, `widget`, `widgetparams`, `position`, `visible`) VALUES
    ...
    $this->execute("DROP TABLE profiles");
    $this->execute("DROP TABLE profiles_fields");
		return true;
	}
